...
if st.session_state.resume_generated:
    ...
    st.info(st.session_state.header)
    ...
    with comp_col1:
        ...
        for skill in st.session_state.comparison[0]:
            st.write(f"• {skill}")
    with comp_col2:
        st.markdown("**Job Requirements**")
        for req in st.session_state.comparison[1]:
            st.write(f"• {req}")
    ...
    st.write(st.session_state.work_experience)
    
    st.subheader("Complete Tailored Resume")
    st.text_area("Copy and edit your tailored resume:", st.session_state.full_resume, height=400)
    ...
    st.text_area("Copy your cover letter:", st.session_state.cover_letter, height=300)

    # Generate PDF downloads
    create_pdf(st.session_state.full_resume, "tailored_resume.pdf")
    create_pdf(st.session_state.cover_letter, "cover_letter.pdf")
