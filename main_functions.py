...
def analyze_resume_and_job(resume, job_description):
    system_message = """
    You are an expert resume analyst and career advisor with decades of experience in HR and recruitment across various industries. Your task is to analyze the provided resume and job description, then create a tailored resume that includes the following sections:

    1. HEADER: The candidate's name and key contact information.

    2. SUMMARY: A custom summary (3-4 sentences) that highlights the candidate's most relevant skills and experiences for this specific job.

    3. SKILLS COMPARISON: A detailed two-column comparison of the candidate's skills and the job requirements, listing at least 5 key points for each. Format this section as follows:
       SKILLS COMPARISON
       
       [Candidate Name] SKILLS & EXPERIENCE     | JOB REQUIREMENTS
       • Skill 1                                | • Requirement 1
       • Skill 2                                | • Requirement 2
       • Skill 3                                | • Requirement 3
       • Skill 4                                | • Requirement 4
       • Skill 5                                | • Requirement 5

    4. EDUCATION: A summary of the candidate's education information.

    5. RELEVANT WORK EXPERIENCE: A summary of the most relevant work experience for this job, focusing on the most recent or most applicable positions.

    Format the resume to fit on one page, using clear section headers and bullet points where appropriate. Ensure each section is clearly labeled with its title (HEADER, SUMMARY, SKILLS COMPARISON, EDUCATION, RELEVANT WORK EXPERIENCE).
    """

    user_message = f"""
    Please create a tailored resume based on the following resume and job description:
    ...
    Provide the tailored resume in a format that can be easily converted to a PDF, with clear section breaks and appropriate formatting. Ensure each section is preceded by its title in all caps.
    """
...
def create_pdf(content, filename):
    ...
    for section in sections:
        ...
        if section_title == "HEADER":
            ...
            header_lines = section_content.split('\n')
            for line in header_lines:
                ...
                pdf.cell(text_width, 6, line, align='C', ln=True)
        else:
            pdf.set_font("Helvetica", 'B', size=11)
            pdf.cell(0, 6, section_title, ln=True)
            pdf.set_font("Helvetica", size=10)
            
            if section_title == "SKILLS COMPARISON":
                comparison_lines = section_content.split('\n')[1:]  # Skip the blank line
                for line in comparison_lines:
                    if "|" in line:
                        parts = line.split("|")
                        pdf.cell(effective_page_width * 0.5, 5, parts[0].strip(), 0, 0)
                        pdf.cell(effective_page_width * 0.5, 5, "| " + parts[1].strip(), 0, 1)
                    else:
                        pdf.cell(0, 5, line, ln=True)
            else:
                pdf.multi_cell(effective_page_width, 5, section_content)
